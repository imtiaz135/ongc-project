...
export const SnippingTool: React.FC<SnippingToolProps> = ({ file, onExtract }) => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    // Adjust for zoom level - divide by zoom scale to get unscaled coordinates
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // Adjust for zoom level - divide by zoom scale to get unscaled coordinates
    const zoomScale = zoomLevel / 100;
    const currX = (e.clientX - rect.left) / zoomScale;
    const currY = (e.clientY - rect.top) / zoomScale;
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center space-x-1 ml-4 pl-4 border-l border-gray-300">
            <button 
              ...
              className="px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium transition-colors shadow-sm"
              title="Zoom Out"
            >
              🔍−
            </button>
            <span className="text-sm text-gray-600 font-medium px-3 py-2 bg-gray-50 rounded-lg min-w-[60px] text-center">
              {zoomLevel}%
            </span>
            <button 
            ...
            <button 
              ...
              className="px-3 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium transition-colors shadow-sm"
              title="Reset Zoom"
            >
              ↺
            </button>
      ...
      <div className="flex-1 overflow-auto bg-gray-100 flex justify-center p-8">
        <div 
          ref={containerRef}
          className="relative shadow-2xl border border-gray-200 bg-white transition-transform"
          style={{ 
