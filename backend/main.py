    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if sel.use_ai:
                ...
                if ollama_data:
                    print(f"DEBUG: Ollama extracted {len(ollama_data)} records from OCR text")
                    ...
                    if manual_data:
                        ...
                        if "_warning" not in manual_data[0]:
                            manual_data[0]["_warning"] = "OCR + fallback text parsing"
                        data.extend(manual_data)
                    else:
                        print("DEBUG: Text parsing failed - returning raw OCR text")
                        data.append({"raw_text": extracted_text[:2000], "_warning": "OCR extraction succeeded but could not parse structure"})
            else:
                # Use Manual parsing
                manual_data = parse_extracted_text(extracted_text)
                if manual_data:
                    ...
                    data.append({"raw_text": extracted_text[:2000], "_warning": "OCR succeeded but no structured data found"})
...
def extract_from_region(pdf_path: str, sel: RegionSelection, use_raw_headers: bool = False) -> List[Dict]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        # def is_valid_table_header(headers_dict):
            """
            Validates if detected headers are actually table column headers vs wrapped text.
            Returns False if headers look like a sentence or paragraph text.
            """
            if not headers_dict:
                return False
            
            header_values = list(headers_dict.values())
            
            # Too many columns (>15) usually means wrapped text was split
            if len(header_values) > 15:
                print(f"DEBUG: Rejecting table - too many columns: {len(header_values)}")
                return False
            
            # Check if headers look like sentence fragments (small words that form a sentence)
            joined_headers = " ".join(header_values)
            
            # Common sentence starters that indicate this is wrapped text, not a table
            sentence_indicators = [
                "the ", "overview", "of the", "is ", "in ", "below", "table",
                "following", "presented", "shown", "contains", "includes"
            ]
            
            lowered = joined_headers.lower()
            sentence_count = sum(1 for indicator in sentence_indicators if indicator in lowered)
            
            # If 4+ sentence indicators found, this is wrapped text, not a table header
            if sentence_count >= 4:
                print(f"DEBUG: Rejecting table - headers look like wrapped text: {header_values}")
                return False
            
            # Check if average header length is too short (real headers are usually 2-30 chars)
            avg_header_length = sum(len(h) for h in header_values) / len(header_values) if header_values else 0
            if avg_header_length < 1:
                print(f"DEBUG: Rejecting table - headers too short (avg: {avg_header_length})")
                return False
            
            # If all headers are single lowercase words, likely wrapped text
            single_word_count = sum(1 for h in header_values if len(h.split()) == 1 and len(h) < 6)
            if single_word_count > len(header_values) * 0.8:  # 80% threshold
                print(f"DEBUG: Rejecting table - mostly single short words: {header_values}")
                return False
        ...
        if tables:
            for table_idx, table in enumerate(tables):
                ...
                # 🔍 STEP 1: Detect if first row is actually a DATA ROW, not headers
                first_row_text = [str(cell).strip() for cell in table[0] if cell]
                numeric_count = sum(1 for cell in first_row_text if any(c.isdigit() for c in str(cell)))
                is_first_row_data = numeric_count > len(first_row_text) * 0.6  # >60% numeric = data row
                
                print(f"DEBUG: First row analysis - numeric_cells={numeric_count}, total_cells={len(first_row_text)}, is_data_row={is_first_row_data}")
                
                if is_first_row_data:
                    print(f"DEBUG: ⚠️ First row appears to be DATA, not headers!")
                    print(f"DEBUG: First row content: {first_row_text[:3]}...")
                    print(f"DEBUG: ⚠️ You must include the actual HEADER ROW in your selection!")
                    print(f"DEBUG: Please reselect the table starting from the row with column names")
                    # Skip this table since headers are missing
                    continue
                ...
                # 📊 SKIP HEADER ROW AND EXTRACT ONLY DATA ROWS
                data_rows = table[1:] if len(table) > 1 else []
                print(f"DEBUG: Found {len(data_rows)} data rows (skipped header)")
